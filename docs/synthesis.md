...
- `--goft-temperature-chunk`: Compute G(T, n_e) this many temperatures at a time instead of the whole grid at once (default: the whole grid). fiasco solves the level populations for all the temperatures it is given together, so for an ion with many levels it can need several GB: Fe XII, with 912 levels in CHIANTI 10.1, needs about 4.5 GB. With `--goft-temperature-chunk 25` it needs about 1.4 GB, and with `10` under 1 GB, and the result is the same. Each chunk reads the atomic data again, so it takes longer: for Fe XII on one of our cluster nodes, 2.5 minutes with chunks of 25 and 5 with chunks of 10, rather than 1. With several ions and `--n-workers`, each worker needs this memory at once.
