...
a synthesis file. The fourth brings in spectra that some other code has already
produced; ECLIPSE does not try to reproduce what those codes do, it just needs to
receive their output. The last has no synthesis step at all and is set directly
...
- **Spectra from any other synthesis code.** ECLIPSE's own synthesis is optically
  thin, and will stay that way. Rather than reimplementing optically thick
  radiative transfer, ECLIPSE will read the spectra produced by codes that
  already do it well - [Lightweaver](https://github.com/Goobley/Lightweaver),
  [RH1.5D](https://rh15d.readthedocs.io/), or whatever you prefer - so the
  instrument simulation can be run on top of any of them.
