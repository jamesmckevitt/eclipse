...
; Author: Mark Cheung
; Revision history: 2016-08-28: v0.8
; Purpose: Read in corona_emission_adj_vlos_<dir>.XXXXXX cubes output from
;          MURaM simulations and convert to Emisssion Measure (EM)
;          weighted Doppler velocity maps
; Usage: IDL> e = hgcr_muram2vlos(340000l, dir='y')
;        IDL> e = hgcr_muram2vlos(340000l, dir='z')
...
function hgcr_muram2vlos, nmod, dir=dir, time=time, lgt=lgt, t6l=t6l, t6r=t6r, t6m=t6m, resample=resample, offset=offset
  if n_elements(dir) EQ 0 then dir = 'y'
  if n_elements(resample) EQ 0 then resample = 0
  ...
  IF KEYWORD_SET(resample) EQ 1 THEN BEGIN
     ...
     for n=0,n_elements(lgt)-1 do begin
        ...
        IF (lgtl_ind GE 0) AND (lgtl_ind LT n_elements(t6l)-1) THEN BEGIN
           ...
           vlosnew[*,*,n] =  emcube[*,*,lgtl_ind]*(1.0-mu_l)+emcube[*,*,lgtl_ind+1]*mu_l
  ...
  return, {t6l:t6l, t6r:t6r, t6m:t6m, vlos:emcube, $
