"""Instrument response simulation for EUVST SW channel.

This module contains utilities to convolve synthetic spectra with the
instrumental response of the Solar-C/EUVST short wavelength channel.
It performs spectral and spatial resampling, applies the telescope PSF
and detector characteristics and provides simple analysis helpers.

The original version of this code was a monolithic script mixing
astropy units and plain numbers.  The new version aims to keep the
same logic while improving readability and unit handling.
"""
...
class Detector:
    ...
    def plate_scale_length(self) -> u.Quantity:
        return angle_to_distance(self.plate_scale_angle) / u.pixel
...
@dataclass
class Simulation:
    expos: u.Quantity = u.Quantity([1, 2, 5, 10, 20], u.s)
    n_iter: int = 50
...
def slit_sample_cube(
    cube_hr: u.Quantity,
    dx_cm: u.Quantity,
    slit_w_cm: u.Quantity,
    scan_step_cm: u.Quantity,
) -> u.Quantity:
    """Slide slit along X and integrate cube into mean specific intensity."""
    Nx, Ny, Nl = cube_hr.shape
    prefix = np.concatenate(
        [np.zeros((1, Ny, Nl), dtype=cube_hr.dtype), np.cumsum(cube_hr, axis=0) * dx_cm],
        axis=0,
    )

    def interp_prefix(pos: u.Quantity) -> u.Quantity:
        pos_idx = (pos / dx_cm).decompose().value
    ...
    left_edges = np.arange(0, (Nx * dx_cm - slit_w_cm).value + 1e-9, scan_step_cm.value) * dx_cm.unit
    ...
    for s, L in enumerate(left_edges):
        ...
        cube_out[s] = (integral / slit_w_cm).value
...
def rebin_atmosphere(
    ...
    wl_pitch_out = detector.wvl_res
    y_pitch_out = detector.plate_scale_length
...
def _find_new_key_idxs(_: u.Quantity, __: u.Quantity, plotting: dict) -> dict:
...
def photons_to_pixel_rate(ph_sr_cm_s: u.Quantity, wl_pitch: u.Quantity, spt_pitch: u.Quantity) -> u.Quantity:
    pixel_solid_angle = ((spt_pitch * u.pixel) ** 2 / const.au ** 2).to(u.sr)
...
def fit_cube_gauss(signal_cube: u.Quantity, wv: u.Quantity, n_jobs: int = -1) -> u.Quantity:
...
def plot_radiometric_pipeline(
    ...
    def spectrum(stage_idx: int, row_idx: int) -> np.ndarray:
        return signals[stage_idx][idxs_reb[row_idx] + (slice(None),)].value
...
def plot_maps(
    ...
    wl0 = wl_axis[len(wl_axis) // 2]
    v_map = (wl_axis[peak_idx] - wl0) / wl0 * const.c.to(u.km / u.s)
    ...
    imV = axV.imshow(v_map.T.value, origin="lower", aspect="equal", cmap="RdBu_r", vmin=-15, vmax=15)
